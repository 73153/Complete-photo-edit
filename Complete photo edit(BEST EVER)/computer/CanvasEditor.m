...
@interface CanvasEditor () <UIGestureRecognizerDelegate> {
    ...
    CGSize _resizeHandleObjectInitialSize;
    CGPoint _resizeHandleObjectInitialCenter;
    CGPoint _resizeHandleInitialTouchPos;
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    [self createObjectMoveTransaction];
...
- (void)createObjectMoveTransaction {
...
- (void)resizeHandleWasPanned:(UIPanGestureRecognizer *)gestureRec drawable:(CMDrawable *)drawable {
    [self createObjectMoveTransaction];
    if (gestureRec.state == UIGestureRecognizerStateBegan || gestureRec.state == UIGestureRecognizerStateChanged) {
        CMDrawableKeyframe *selectionKeyframe = [[self.singleSelection.keyframeStore createKeyframeAtTimeIfNeeded:self.time] copy];
        CMDrawableView *view = [self.canvasView viewForDrawable:self.singleSelection];
        CGSize currentInternalSize = CMSizeWithDiagonalAndAspectRatio(self.singleSelection.boundsDiagonal, self.singleSelection.aspectRatio);
        
        if (gestureRec.state == UIGestureRecognizerStateBegan) {
            _resizeHandleObjectInitialSize = currentInternalSize;
            _resizeHandleObjectInitialCenter = selectionKeyframe.center;
            _resizeHandleInitialTouchPos = [gestureRec locationInView:view];
        }
        CGPoint pos = [gestureRec locationInView:view];
        CGSize newInternalSize = CGSizeMake(_resizeHandleObjectInitialSize.width + pos.x - _resizeHandleInitialTouchPos.x, _resizeHandleObjectInitialSize.height + pos.y - _resizeHandleInitialTouchPos.y);
        
        CGFloat newAspectRatio = (newInternalSize.width * newInternalSize.height) ? newInternalSize.width / newInternalSize.height : 1;
        CGFloat newBoundsDiagonal = sqrt(pow(newInternalSize.width, 2) + pow(newInternalSize.height, 2));
        // CGPoint newCenter = CGPointMake(_resizeHandleObjectInitialCenter.x + (newInternalSize.width - _resizeHandleObjectInitialSize.width)/2, _resizeHandleObjectInitialCenter.y + (newInternalSize.height - _resizeHandleObjectInitialSize.height)/2);
        
        // selectionKeyframe.center = newCenter;
        
        _currentObjectMoveTransaction.action = ^(id target) {
            [drawable.keyframeStore storeKeyframe:selectionKeyframe];
            drawable.boundsDiagonal = newBoundsDiagonal;
            if ([drawable respondsToSelector:@selector(setAspectRatio:)]) {
                [(id)drawable setAspectRatio:newAspectRatio];
            }
        };
    }
}
...
- (void)renderSelectionIndicatorsInContext:(CMRenderContext *)ctx {
    ...
    BOOL showResizeHandle = selectedItems.count == 1 && [selectedItems.firstObject showResizeHandleWhenSelected];
    ...
    for (NSInteger i=0; i<_selectionViews.count; i++) {
        ...
        selectionView.showResizeHandle = showResizeHandle;
        CMDrawable *d = selectedItems[i];
        selectionView.representsDrawable = d;
