...
- (void)viewDidLoad {
    ...
    self.alignmentButton = [UIButton buttonWithType:UIButtonTypeCustom];
    // initialize with the word 'ALIGNMENT'; in reality, we just need a word wide enough for the widest alignment name (Left/Center/Right)
    [self.alignmentButton setTitle:NSLocalizedString(@"Alignment", @"").uppercaseString forState:UIControlStateNormal];
    [self.alignmentButton addTarget:self action:@selector(changeAlignment) forControlEvents:UIControlEventTouchUpInside];
    
    self.navigationItem.leftBarButtonItems = [@[font, color, shadow, self.alignmentButton] map:^id(id obj) {
        UIButton *b = obj;
...
- (void)typingAttributesChanged {
    ...
    NSParagraphStyle *para = self.textView.typingAttributes[NSParagraphStyleAttributeName];
    if (para) {
        alignment = para.alignment;
    }
    NSDictionary *alignmentNames = @{
                                     @(NSTextAlignmentLeft): @"Left",
                                     @(NSTextAlignmentCenter): @"Center",
                                     @(NSTextAlignmentRight): @"Right"
                                     };
    [self.alignmentButton setTitle:[(alignmentNames[@(alignment)] ? : @"Left") uppercaseString] forState:UIControlStateNormal];*/
}
...
- (void)changeAlignment {
    ...
    NSArray *alignments = @[@(NSTextAlignmentLeft), @(NSTextAlignmentCenter), @(NSTextAlignmentRight)];
    NSInteger index = 0;
    if ([alignments containsObject:@(alignment)]) {
        index = [alignments indexOfObject:@(alignment)];
    }
    NSTextAlignment newAlignment = [alignments[(index+1) % alignments.count] integerValue];
    [self updateTextEntryAttribute:NSParagraphStyleAttributeName function:^id(id existing) {
        NSMutableParagraphStyle *para = [(existing ? : [NSParagraphStyle defaultParagraphStyle]) mutableCopy];
