...
- (CMDrawableView *)renderToView:(CMDrawableView *)existingOrNil context:(CMRenderContext *)ctx {    
    ...
    for (CMDrawable *drawable in self.contents) {
        ...
        if (newView == existing) {
            if (![newView superview]) [v addSubview:newView];
            [v bringSubviewToFront:newView];
        } else {
            if (existing.superview == v) [existing removeFromSuperview];
            [v addSubview:newView];
        }
        v.viewsByKey[drawable.key] = newView;
        
        if (self.viewDisplaySuppressionBlock && self.viewDisplaySuppressionBlock(drawable)) [newView removeFromSuperview];
    }
