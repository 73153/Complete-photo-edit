...
- (void)viewDidLoad {
    ...
    text.action = ^{
        ...
        NSDictionary *attrs = @{NSFontAttributeName: [UIFont fontWithName:@"Avenir-Medium" size:14], NSParagraphStyleAttributeName: para};
    ...
    QuickCollectionItem *pixelArt = [QuickCollectionItem new];
    pixelArt.label = NSLocalizedString(@"Pixels", @"");
    pixelArt.action = ^{
        CMPixelArtDrawable *d = [CMPixelArtDrawable new];
        [weakSelf.editorVC.canvas insertDrawableAtCurrentTime:d];
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [d performDefaultEditActionWithEditor:weakSelf.editorVC];
        });
    };
    ...
    QuickCollectionItem *flipbook = [QuickCollectionItem new];
    flipbook.label = NSLocalizedString(@"Flipbook", @"");
    flipbook.action = ^{
        CMFlipbook *f = [CMFlipbook new];
        [weakSelf.editorVC.canvas insertDrawableAtCurrentTime:f];
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [weakSelf.editorVC showPropertyEditors];
        });
    };
    
    self.items = @[camera, photos, imageSearch, text, pen, circle, square, star, particle, flipbook, pixelArt, exportCamera];
}
