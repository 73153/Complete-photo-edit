...
+ (EditorViewController *)modalEditorForCanvas:(CMCanvas *)canvas callback:(void(^)(CMCanvas *edited))callback {
    ...
    if (CGRectIsNull(bounds) || isnan(bounds.origin.x) || isnan(bounds.origin.y)) {
        bounds = CGRectMake(0, 0, 400, 400);
    }
...
- (void)beginExportFlow {
    ...
    [ac addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"⚡️ Save Photo to Camera Roll", @"") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        PhotoExporter *p = [PhotoExporter new];
        p.quickSaveToCameraRoll = YES;
        [weakSelf startCroppingWithExporter:p];
    }]];
    [ac addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"⚡️ Share GIF as a Link", @"") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        GifExporter *g = [GifExporter new];
        g.goStraightToURLShare = YES;
        [weakSelf startCroppingWithExporter:g];
    }]];
    [ac addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"📲 Make iMessage Sticker", @"") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        PhotoExporter *p = [PhotoExporter new];
        p.saveAsSticker = YES;
        [weakSelf startCroppingWithExporter:p];
    }]];
    [ac addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Share Photo", @"") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [weakSelf startCroppingWithExporter:[PhotoExporter new]];
    }]];
    [ac addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Share Video", @"") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [weakSelf startCroppingWithExporter:[VideoExporter new]];
    }]];
    [ac addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Share GIF", @"") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [weakSelf startCroppingWithExporter:[GifExporter new]];
    }]];
    [ac addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Never mind", @"") style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
